...
const PublicMatchCard = ({
  ...
  const renderTeamRow = (name: string, logo: string | undefined, teamId: string | null, penalties: number | null, score: number | null, isWin: boolean) => (
    <div className={`flex items-center gap-2 ${ds(bStyle, "matchTeamRow") || "rounded-md"} px-2 py-1.5 transition-colors`}>
      ...
      <div className={`grid items-center gap-1 ${hasPenalties ? "grid-cols-[1fr_2.5rem_1.25rem]" : "grid-cols-[1fr_2.5rem]"}`}>
        {m.is_played ? (
          <>
            <span className={`text-right tabular-nums ${ds(bStyle, "matchScore")} ${isWin ? "font-bold " + ds(bStyle, "matchScoreWin") : ds(bStyle, "matchScoreLose")}`}>{score}</span>
            {hasPenalties && (
              <span className="text-left text-[8px] font-medium leading-none text-muted-foreground">({penalties})</span>
            )}
          </>
        ) : null}
      </div>
